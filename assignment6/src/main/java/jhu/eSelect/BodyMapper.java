 ...
 * 2018/11/15
 */
public class BodyMapper extends Mapper<AvroKey<EmailSimple>, NullWritable,
    ...
    protected void map(AvroKey<EmailSimple> key, NullWritable value, Context context) throws IOException, InterruptedException {
       EmailSimple email = key.datum();
        ...
        CharSequence bodychar = email.getBody();
        
        String bodyText="";
        
        if (bodychar!=null)
        {
            bodyText = email.getBody().toString();


            if(bodyText.matches(regex))
            {

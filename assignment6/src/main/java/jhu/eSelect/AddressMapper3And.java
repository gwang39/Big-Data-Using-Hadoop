 ...
 * 2018/11/15
 */
public class AddressMapper3And extends Mapper<AvroKey<EmailSimple>, NullWritable,
    ...
    protected void setup(Context context) throws IOException, InterruptedException {
        ...
        this.regex3 = context.getConfiguration().get("regex3");
        this.sOption3 = context.getConfiguration().get("sOption3");
    ...
    protected void map(AvroKey<EmailSimple> key, NullWritable value, Context context) throws IOException, InterruptedException {
        ...
        if (sOption3.equals("-from") && from!=null)
        {
                if(from.toString().matches(regex3))
                {
                    includeFlag3 = true;
                }
        }
        else if (sOption3.equals("-to") && to!=null)
        {
            for(CharSequence e : to)
            {
                if(e.toString().matches(regex3))
                {
                    includeFlag3 = true;
                }
            }
        ...
        else if (sOption3.equals("-cc") && cc!=null)
        {
            for(CharSequence e : cc)
            {
                if(e.toString().matches(regex3))
        ...
        if (includeFlag1 && includeFlag2 && includeFlag3)
        {
